...
class ConfigFrame(tk.LabelFrame):
    ...
    def __init__(self, master, config):
        super().__init__(master, text=" 基本配置 ", font=("Microsoft YaHei UI", 10, "bold"), padx=15, pady=10)
    ...
    def _setup_ui(self):
        # Proxifier 路径
        tk.Label(self, text="Proxifier 可执行文件路径:", font=("Microsoft YaHei UI", 9)).pack(anchor="w")
        ...
        tk.Button(path_frame, text="浏览", command=self._browse_file, width=8).pack(side=tk.RIGHT)
        
        # 服务名称
        tk.Label(self, text="驱动服务名称:", font=("Microsoft YaHei UI", 9)).pack(anchor="w")
        tk.Entry(self, textvariable=self.service_var, font=("Consolas", 9)).pack(fill="x", pady=2)
        tk.Label(self, text="* 通常为 'proxifierdrv'，不熟悉请勿修改", font=("Microsoft YaHei UI", 8), fg="gray").pack(anchor="w")
